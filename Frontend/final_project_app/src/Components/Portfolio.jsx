...
function Portfolio() {
    ...
    const [netGainLoss, setNetGainLoss] = useState(0); // State to store net gain/loss
    const [stockPrices, setStockPrices] = useState({}); // Store stock prices for each ticker
    ...
    const handleToggleTransactionHistory = (ticker) => {
        ...
        if (ticker === selectedTicker) {
            setNetGainLoss(0); // Reset net gain/loss when hiding transaction history
        } else {
            // Fetch stock price and calculate net gain/loss when showing transaction history
